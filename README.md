...
Run `arbiter` with no arguments for guided setup, or select a harness directly:

```bash
arbiter
arbiter onboard --harness opencode
...
Release archives and the installer include both the guided `arbiter` CLI and
the `arbiter-mcp` gateway used by MCP-based harness onboarding.
